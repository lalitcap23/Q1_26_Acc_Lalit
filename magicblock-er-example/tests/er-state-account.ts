...
describe("er-state-account", () => {
  ...
  it("Call VRF on base layer", async () => {
    await program.methods.requestRandomness()
      .accountsPartial({
        ...
        oracle: new PublicKey("Cuj97ggrhhidhbu39TijNVqE74xvKJ69gDervRUXAxGh"),
        user: userAccount,
        vrfProgram: new PublicKey("Vrf1RNUjXmQGjmQrQLvJHs9SNkvDJEsRVFPk9SQUwGz"),
        systemProgram: anchor.web3.SystemProgram.programId,
      }).rpc();
  ...
  it("Execute VRF Delegated", async () => {
    let tx = await ephemeralProgram.methods.requestRandomness().accountsPartial({
      signer: anchor.Wallet.local().publicKey,
      oracle: new PublicKey("5hBR571xnXppuCPveTrctfTU7tJLSN94nq7kv7FRK5Tc"),
      user: userAccount,
      vrfProgram: new PublicKey("Vrf1RNUjXmQGjmQrQLvJHs9SNkvDJEsRVFPk9SQUwGz"),
      systemProgram: anchor.web3.SystemProgram.programId,
    }).transaction();
